...
object Day1 extends App {
  ...
  def calculate_required_fuel(mass: Int): Int = {
    ...
    def calculate_amount(current_mass: Int, acc_fuel: Int): Int = {
      val current_fuel = math.floor(current_mass / 3).toInt - 2
      if (current_fuel > 0) {
        return calculate_amount(current_fuel, acc_fuel + current_fuel)
      } else {
        return acc_fuel
      }
    }

    return calculate_amount(mass, 0)
